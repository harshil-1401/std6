...
function shuffleMixed<T extends { magnetic: boolean }>(items: T[]): T[] {
  const fy = (arr: T[]) => {
    const a = arr.slice();
...
function MagnetSortGame({ props = {} }: ToolProps) {
  ...
  const [order, setOrder] = useState<ObjectDef[]>(() => shuffleMixed(config.objects));
  ...
  const startGame = useCallback(() => {
    clearTimers();
    const fresh = shuffleMixed(config.objects);     // fresh random order every game — no pattern
    setOrder(fresh);
  ...
  // ---- tap to test (mobile / accessibility) ----
  const tapObject = () => {
  ...
  useEffect(() => {
    if (current === null && testedCount === TOTAL && !showResults) {
      const t = window.setTimeout(() => setShowResults(true), 420);
  ...
  return (
    <div style={St.root}>
      ...
      <div style={St.frame}>
        ...
        <div style={St.body}>
          <p style={St.instruction}>{config.instruction}</p>
