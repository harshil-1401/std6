/* This renderer strips all `import` statements and provides React globally
   ...
   const TOOL_ID = "sieve_matching_board";
   ...
   function SieveMatchingBoard({ props = {}, setStepDetails }: ToolProps) {
     ...
     live.current = { phase, exampleIdx, picks, status, mixtures, exploreMixtures, options };
     ...
     const goToMixture = (mixtureId: string) => {
       const idx = live.current.exploreMixtures.findIndex((mm: Mixture) => mm.id === mixtureId);
     ...
     const snapshot = () => ({
       ...
       currentMixture: live.current.exploreMixtures[live.current.exampleIdx]?.id ?? null,
       picks: { ...live.current.picks },
     ...
     const api = {
       ...
       prev: () => {
         const i = live.current.exampleIdx;
         if (i > 0) { setExampleIdx(i - 1); emitEvent("example_selected", { mixtureId: live.current.exploreMixtures[i - 1].id }); }
         return { ok: true };
       },
       pickSieve: (mixtureId: string, sieveId: string) => {
         const m = live.current.exploreMixtures.find((mm: Mixture) => mm.id === mixtureId);
         if (!m) return { ok: false, reason: "unknown mixture" };
         if (!live.current.options.some((o: SieveOption) => o.id === sieveId)) return { ok: false, reason: "unknown sieve" };
         goToMixture(mixtureId);
         return handlePick(m, sieveId);
       },
     };
